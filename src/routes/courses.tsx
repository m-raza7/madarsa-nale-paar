...
export const Route = createFileRoute("/courses")({
  head: () => ({
    meta: [
      { title: "Courses — Madarsa NALE-PAAR" },
      {
        name: "description",
        content:
          "Hifz, Nazra, Tajweed, Alim Course, Arabic Language and Islamic Studies — full curricula.",
      },
      { property: "og:title", content: "Our Courses" },
      {
        property: "og:description",
        content: "Programs from beginner Nazra to the 8-year Alim Course.",
      },
    ],
  }),
  component: Courses,
    ...
    <div>
      <section className="bg-primary-gradient text-primary-foreground py-16">
        <div className="container mx-auto px-4 text-center">
          ...
          <p className="text-white/85 mt-4 max-w-2xl mx-auto">
            From beginner Quranic reading to advanced Islamic scholarship.
          </p>
        </div>
      </section>
      ...
      <section className="container mx-auto px-4 py-16 grid md:grid-cols-2 lg:grid-cols-3 gap-6">
        {courses.map((c) => (
          <Card
            key={c.id}
            className="p-7 group hover:shadow-elegant hover:-translate-y-1 transition-all"
          >
